...
export const TOOLS = [
    ...
    { name: "device_start", description: "Start an owner-scoped device instance lazily", inputSchema: { type: "object", properties: deviceBrokerProperties({ deviceId: DEVICE_ID_PROPERTY, incarnationId: INCARNATION_ID_PROPERTY, waitForBoot: { type: "boolean", description: "Skip the boot-readiness wait. Rejected for linux-vm and windows-vm start/reboot, where readiness is what tears down bootstrap networking and removes the provisioning media." }, bootTimeoutMs: HYPER_V_BOOT_TIMEOUT_PROPERTY, headless: { type: "boolean" }, minimized: { type: "boolean" } }), required: ["deviceId"] } },
    ...
    { name: "device_reboot", description: "Reboot an owner-scoped VM and wait for its guest transport to become ready", inputSchema: { type: "object", properties: typedDeviceBrokerProperties(REBOOT_VM_BACKEND_PROPERTY, { deviceId: DEVICE_ID_PROPERTY, incarnationId: INCARNATION_ID_PROPERTY, force: { type: "boolean" }, startIfStopped: { type: "boolean" }, waitForBoot: { type: "boolean", description: "Skip the boot-readiness wait. Rejected for linux-vm and windows-vm start/reboot, where readiness is what tears down bootstrap networking and removes the provisioning media." }, bootTimeoutMs: HYPER_V_BOOT_TIMEOUT_PROPERTY }), required: ["deviceId"] } },
    { name: "device_status", description: "Inspect an owner-scoped device definition or instance", inputSchema: { type: "object", properties: typedDeviceBrokerProperties(DEVICE_STATUS_BACKEND_PROPERTY, { deviceId: DEVICE_ID_PROPERTY, incarnationId: INCARNATION_ID_PROPERTY }), required: ["deviceId"] } },
