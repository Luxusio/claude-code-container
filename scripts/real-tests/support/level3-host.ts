...
export const HYPER_V_LEVEL3_REQUIRED_BROKER_CAPABILITIES = [
    ...
    "hyper-v-provider-image-finalization-v3",
    "hyper-v-network-failure-diagnostics-v9",
];
export const HYPER_V_LEVEL3_PROVIDER_CONTRACT = "hyper-v-provider-image-finalization-v3";
export const HYPER_V_LEVEL3_NETWORK_OWNERSHIP_CONTRACT = "hyper-v-setup-network-v9";
...
export async function ensureHostBrokerReady(repoRoot, options: any = {}) {
    ...
    if (result.status === 0 && /brokerReady:\s*true/.test(stdout)) {
        ...
        if (confirmed.status === 0
            ...
            process.stdout.write(`ATTEST Hyper-V broker pid=${observed.pid} startedAt=${observed.startedAt} providerContract=${HYPER_V_LEVEL3_PROVIDER_CONTRACT} networkOwnership=${HYPER_V_LEVEL3_NETWORK_OWNERSHIP_CONTRACT} networkDiagnostics=${HYPER_V_LEVEL3_NETWORK_DIAGNOSTICS_CONTRACT}\n`);
            return 0;
        }
        process.stderr.write([
