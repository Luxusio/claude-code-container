...
import { freePort, localCccPathEnv } from "./helpers.mjs";
import { markExpectedFlowStepErrors, markExpectedToolError, parseToolPayload, withDeviceLabMcp } from "./device-lab-mcp-client.mjs";
import { installedMcpSmokeSample } from "./installed-mcp-smoke.mjs";
...
export async function runBrokerE2E(options = {}) {
    ...
    try {
        await withDeviceLabMcp(async ({ callTool }) => {
            ...
            try {
            const backends = parseToolPayload(await callTool("device_backends", publicRoute));
            ...
            try {
            const inventory = parseToolPayload(await callTool("device_inventory", { ...publicRoute, backend: "android-emulator" }));
