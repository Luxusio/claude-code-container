...
import { PRIVILEGED_RESULT_MARKER, type PrivilegedSetupDiagnosticsInput } from "./hyper-v-windows-setup-diagnostics-privileged.ts";
// Deliberately NOT HyperVWindowsSetupDiagnosticsResult: its success variant carries absolute host
// paths, and this boundary does not transport them. Widening the type to fit would have meant
// returning empty strings for two fields, which reads as data and is not.
export type ElevatedSetupDiagnosticsResult =
    | { ok: true; latestRelativePath: string }
...
const MAX_RESULT_FRAME_CHARS = 8 * 1024;
...
export type ElevatedSetupDiagnosticsOutcome =
    | { attempted: false; reason: "not-windows" | "already-elevated" | "bundle-missing" | "probe-failed" }
...
// JSON.stringify of a validated object cannot break out of the expression: every value is a string
// matching one of the identity patterns, and stringify escapes what remains.
...
export function decodePrivilegedResultFrame(stdout: string): ElevatedSetupDiagnosticsResult | null {
    ...
    if (parsed.ok === true) {
        // The relative path is re-checked rather than trusted: it is the one field that reaches the
        // operator verbatim, and it arrives from a process running as Administrator.
        return typeof parsed.latestRelativePath === "string" && /^[A-Za-z0-9][A-Za-z0-9._/-]{0,255}$/.test(parsed.latestRelativePath)
            ? { ok: true, latestRelativePath: parsed.latestRelativePath }
            : null;
    }
    if (parsed.ok === false) {
        return typeof parsed.code === "string" && /^[a-z0-9][a-z0-9-]{0,127}(\[[^\]\n]{0,1024}\])?$/.test(parsed.code)
