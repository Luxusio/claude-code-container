...
const assertJsonPath = resolve(scriptPath, "../assert-json.mjs");
const requiredToolSources = ["source", "dist"];
const requiredProviderSources = ["dist"];

export const PROVIDER_RESULT_SPECS = [
    { id: "android-emulator", files: ["level2-android-emulator-e2e.mjs", "level3-real-destructive.mjs"], tools: androidBackend().capabilities },
    { id: "android-device", files: ["level2-android-device-e2e.mjs"], tools: ["device_attach", "device_status", "mobile_tap", "mobile_screenshot", "device_detach"] },
    { id: "ios-simulator", files: ["level2-ios-e2e.mjs"], tools: ["device_create", "device_start", "mobile_tap", "mobile_screenshot", "device_delete"] },
    { id: "ios-device", files: ["level2-ios-e2e.mjs"], tools: ["device_attach", "device_status", "mobile_tap", "mobile_screenshot", "device_detach"] },
    { id: "windows-sandbox", files: ["level2-windows-sandbox.mjs"], tools: windowsBackend().capabilities },
    { id: "macos-vm", files: ["level3-real-destructive.mjs"], tools: ["device_base_image_create", "device_base_image_clone", "device_snapshot_create", "device_snapshot_restore", "device_snapshot_delete"] },
    { id: "android-wireless", backend: "android-device", filesBySource: { source: ["level1-real-provider-readiness.mjs"], dist: ["level1-dist-real-provider-readiness.mjs"] }, tools: ["device_wireless"] },
    { id: "ios-wireless", backend: "ios-device", filesBySource: { source: ["level1-real-provider-readiness.mjs"], dist: ["level1-dist-real-provider-readiness.mjs"] }, tools: ["device_wireless"] },
];
...
export function assertResultMatrix(shards, options = {}) {
    ...
    const linuxVmRecords = shards.flatMap((shard) => shard.records || []).filter((record) => (
        basename(String(record?.file || "").replace(/\\/g, "/")) === "level2-real-linux-vm.mjs"
        && record?.status === "PASS"
    ));
...
if (resolve(process.argv[1] || "") === resolve(scriptPath)) {
    ...
    if (files.length === 0) {
        console.error("Usage: node scripts/real-tests/assert-matrix.mjs <level3-result.json> [...level3-result.json]");
        process.exit(1);
    }
    const shards = [];
    ...
    const output = assertResultMatrix(shards);
