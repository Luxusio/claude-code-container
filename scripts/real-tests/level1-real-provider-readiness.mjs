...
import { parseToolPayload, parseToolResult, withDeviceLabMcp } from "./device-lab-mcp-client.mjs";
...
export async function runRealProviderReadiness(options = {}) {
    ...
    await withDeviceLabMcp(async ({ callTool }) => {
        if (commandPath("adb")) {
            const androidWireless = parseToolPayload(await callTool("device_wireless", {
