...
// The asset itself is unchanged and still one-operation-per-invocation: the loop lives here, and
// each iteration re-invokes it with a fresh $global:CccHyperVJsonInput. That is deliberate — the
// pinned asset stays the same artifact the one-shot transport verifies, so both paths execute
// byte-identical operation code and cannot diverge.
export const HYPER_V_WINDOWS_SESSION_BOOTSTRAP = [
    ...
    // Each operation runs in its own child scope with its own output capture, so one failure cannot
    // leave state behind for the next request. Exit codes are meaningless here — the envelope the
    // asset writes carries ok/errorCode, and that is what the reader parses.
    "  $Captured = ''",
    "  try { $Captured = (& $Operation 2>&1 | Out-String) } catch { $Captured = '' }",
    "  $Reply = [ordered]@{ id = [string]$Envelope.id; stdout = [string]$Captured }",
...
export function createHyperVWindowsPowerShellSession(
    ...
    function handleLine(line: string) {
        ...
        let reply: { id?: unknown; stdout?: unknown };
        try {
        ...
        pending.delete(id);
        entry.resolve({ status: 0, stdout: typeof reply.stdout === "string" ? reply.stdout : "" });
    }
    ...
    async function ensureChild(): Promise<HyperVWindowsSessionProcess | null> {
        ...
        if (starts >= maximumStarts) return null;
        starts += 1;
        starting = (async () => {
