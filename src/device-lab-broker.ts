...
export function hyperVProviderDeadlineAt(
    ...
    // windows-vm joins linux-vm here because it now has containment of its own to run after
    // readiness. Without the reserve, a bootTimeoutMs near the residual budget lets
    // assertHyperVOperationDeadline throw right after the readiness runner returns, which REPLACES
    // hyperVGuestReadyExecution with hyper-v-operation-deadline-exceeded. The un-scrubbed reason is
    // gone by the time containment reads it, so containment never runs, scrubContainmentFailed is
    // never set either, and a guest with a live autologon stays up with nothing saying so.
    const needsCleanupReserve = command === "device_create"
        || ((backend === "linux-vm" || backend === "windows-vm")
...
export function hyperVLifecycleCleanupTimeoutMs(
    ...
    return operationTimeoutMs + ((backend === "linux-vm" || backend === "windows-vm")
        && (command === "device_start" || command === "device_reboot")
...
// v22: waitForBoot:false is rejected for windows-vm as it already was for linux-vm — readiness is
// the only place the provisioning media is removed and the only place containment runs, so
...
// requires must appear here. Adding a capability to a required list without adding it here
// passes every other check and then fails every attestation.
export const DEVICE_BROKER_IMPLEMENTED_CAPABILITIES = [
    "http-health",
    "http-status",
    "http-owner-rpc",
    "owner-token-guard",
...
// What the ccc CLI requires OF a remote broker, checked against that broker's advertised /status
// list. Exported alongside DEVICE_BROKER_IMPLEMENTED_CAPABILITIES so a test can prove this one is a
// subset of that one: bumping or adding here without adding there silently breaks every level-3
// run, because the broker keeps answering and the failure surfaces later as an unrelated-looking
// provider error.
...
export async function readHostBrokerHttpJson(response: HostBrokerHttpResponse, maxBytes: number) {
    ...
    // For the RPC lane this precheck and the streaming counter below measure the same bytes; for
    // the three probe callers still on `fetch` they do not. undici sends its own accept-encoding
    // and decompresses, so there a declared length is compressed bytes while `total` counts
    // decompressed ones — measured at 115 against 50,032 for one reply. Never a hole, since the
    // streaming counter is the real bound and catches what the precheck lets through, but on those
    // callers the cheap early rejection cannot fire on a response that would expand past the cap.
    // Nothing diverges today either way: the broker sets no content-encoding anywhere, and
    // compression additionally needs the server to opt in, which it never does.
...
async function invokeHyperVDeviceTool(ownerId: string, parsed: DeviceToolParamSuccess, match: DeviceToolMatch, normalized: NormalizedBrokerOptions): Promise<BrokerRpcResult> {
    ...
    try {
        observation = parsed.tool === "device_snapshot_create"
            ...
            : parsed.tool === "device_snapshot_restore"
                ? await restoreDeviceLabHyperVSnapshot(snapshotRecording.client, snapshotTarget, {
                    ...
                    startAfterRestore: match.backend === "linux-vm",
                })
        ...
        // redactProviderCommandInput reads; the adapter's return type deliberately does not name
        // them, so the broker restates the shape it already owns here.
        const execution = (snapshotRecording.lastExecution() ?? { mode: "exec", provider: "hyper-v", status: null }) as ProviderCommandResult;
        ...
        return {
            ...
            payload: {
                ...
                execution: redactProviderCommandInput(
                    execution,
                    true,
                    hyperVBoundedErrorCode(error, "hyper-v-snapshot-provider-failed"),
                ),
    ...
    if (!transition.matched) {
        if (parsed.tool === "device_snapshot_create") {
            const rollbackRecording = createRecordingDeviceLabHyperVWindowsClient({
                executable: powershell,
                timeoutMilliseconds: 120000,
                run: (command, options) => hyperVProviderCommandRunner(normalized, command, options),
            ...
            try {
                rollbackObservation = await deleteDeviceLabHyperVSnapshot(rollbackRecording.client, {
                    vmId,
            ...
            const rollback = (rollbackRecording.lastExecution() ?? { mode: "exec", provider: "hyper-v", status: null }) as ProviderCommandResult;
    ...
    return {
        ...
        payload: {
            ...
            result: {
                ...
                // The last primitive the adapter issued for this tool call, kept so the success
                // payload keeps reporting a provider execution exactly as it did before.
                execution: redactProviderCommandInput(
                    (snapshotRecording.lastExecution() ?? { mode: "exec", provider: "hyper-v", status: 0 }) as ProviderCommandResult,
                    true,
                ),
                startsDevices: false,
            },
...
function validateCommandParams(params: unknown): CommandParamError | CommandParamSuccess {
    ...
    // windows-vm joins linux-vm here because readiness is now the only place the provisioning
    // media is removed and the only place containment runs. `waitForBoot: false` skipped the whole
    // readiness block, so a plain `device_start { waitForBoot: false }` returned 200 success with
    // the guest Running and CCC_UNATTEND still mounted — a DVD carrying the local Administrator
    // password as PlainText, readable by anything inside the guest. No failure required, and
    // nothing reported it, because on the success path there is no state write to carry a warning.
    // Earlier contracts removed the media on the first successful readiness, so moving the scrub
    // guarantee onto readiness is what turned this from a slow path into an open door.
    if ((backend === "linux-vm" || backend === "windows-vm")
        && (command === "device_start" || command === "device_reboot")
...
type HyperVGuestReadinessFailureDetail = {
    ...
    // reason and containment was NOT CONFIRMED. That is broader than "still running" — the stop
    // may never have been attempted (the cleanup deadline can throw before it), or it may have
    // powered the guest off but returned a confirmation that did not parse or did not match the
    // expected vmId/vmName. Every one of those is treated as unconfirmed, which is the fail-closed
    // reading; what it licenses is "go look", not "it is definitely still up". Omitted otherwise so
    // its presence always means something needs attention, rather than a `false` nobody reads.
...
async function lifecycleCommandInvokeUnlocked(
    ...
    if (success && parsed.backend === "windows-vm" && (parsed.command === "device_start" || parsed.command === "device_reboot") && parsed.waitForBoot !== false) {
        ...
        try {
            ...
            // Never overwrite a readiness result that already arrived. The deadline assert fires
            // AFTER the runner returns, so replacing unconditionally discarded a structured
            // hyper-v-guest-provisioning-not-scrubbed / -first-logon-incomplete and reported
            // hyper-v-operation-deadline-exceeded instead — containment switches on that reason,
            // so an un-scrubbed guest was left running and unreported at exactly the moment the
            // host was most contended.
    ...
    if (success && parsed.backend === "linux-vm" && (parsed.command === "device_start" || parsed.command === "device_reboot") && parsed.waitForBoot !== false) {
        ...
        if (!ssh) {
            ...
            const bootstrapHostKeyAdopted = false;
            ...
            try {
                ...
                do {
                    let networkFinalizedThisIteration = false;
                    attempts += 1;
                    ...
                    if (!bootstrapFinalizationAttempted && Date.now() < deadline) {
                        ...
                        for (const bootstrapAddress of bootstrap?.addresses || []) {
                            ...
                            networkFinalizedThisIteration = true;
                            break;
                        }
                    }
                    if (networkFinalizedThisIteration) continue;
            ...
            mutateOwnerDevices(ownerId, parsed.stateKey, (devices) => devices.map((candidate) => {
                ...
                updatedDevice = {
                    ...
                    lastBootCheck: {
                        ...
                        // The flag has to live in the persisted record, not only in the HTTP
                        // response. The response can be lost — the MCP caller's own timeout can
                        // expire while containment is still running — and this is the one signal
                        // that a guest may still be up with a live autologon. `device_status`
                        // later must be able to see it.
                        ...(hyperVScrubContainmentFailed ? { scrubContainmentFailed: true } : {}),
                        // The diagnostic above is captured BEFORE containment, so on a contained
                        // failure it reports Running while runtimeState says Off. That pair is not
                        // a contradiction to reconcile by moving the capture: taking the diagnostic
                        // after containment would make every contained case a post-mortem of a
                        // machine we had just killed, which is the diagnosability this narrow scope
                        // exists to preserve.
                    },
                    updatedAt: new Date().toISOString(),
                };
