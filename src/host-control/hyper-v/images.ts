...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "    if ([long]$ProfileDrive.AvailableFreeSpace -lt ([long]$QemuOutputItem.Length + [long]2GB)) { throw 'hyper-v-host-disk-capacity-exceeded' }",
        "    $QemuOutputGuard = [IO.File]::Open($QemuOutputPath, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::Read)",
        "    $NormalizedOutput = [IO.File]::Open($PartialPath, [IO.FileMode]::CreateNew, [IO.FileAccess]::Write, [IO.FileShare]::None)",
        ...
        "    $NormalizedItem = Get-Item -LiteralPath $PartialPath -Force -ErrorAction Stop",
        ...
        "    $NormalizedVhd = Get-VHD -Path $PartialPath -ErrorAction Stop",
