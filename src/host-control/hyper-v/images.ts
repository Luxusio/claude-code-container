...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "  if ($Stage -notmatch '^hyper-v-base-image-(download|hash|archive-check|extract|normalize|source-open|source-hash|source-inspection|copy|destination-create|convert|content-verify|partial-open|partial-hash|partial-inspection|final-move|final-inspection|final-observation|work-cleanup)-failed$') { throw 'hyper-v-diagnostic-stage-invalid' }",
        ...
        "$CompareSourceGuard = $null",
        "$CompareTargetGuard = $null",
        "$CompareTargetExpectedHash = $null",
        "$FinalGuard = $null",
        ...
        "    $PartialHashBefore = if ($CompareTargetGuard) { Get-CccGuardedSha256 $CompareTargetGuard } else { (Get-FileHash -LiteralPath $PartialPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant() }",
        "    if ($CompareTargetExpectedHash -and $PartialHashBefore -ne $CompareTargetExpectedHash) { throw 'hyper-v-base-image-partial-mutated' }",
        "    Set-CccAcquireStage 'hyper-v-base-image-partial-inspection-failed'",
        "    $Vhd = Assert-BaseVhd $PartialPath",
        "  } finally { if ($PartialGuard) { $PartialGuard.Dispose(); $PartialGuard = $null } }",
        "  $Generation = $ExpectedGeneration",
        "  Set-CccAcquireStage 'hyper-v-base-image-partial-open-failed'",
        "  if (-not $CompareTargetGuard) { $PartialGuard = [IO.File]::Open($PartialPath, [IO.FileMode]::Open, [IO.FileAccess]::Read, ([IO.FileShare]::ReadWrite -bor [IO.FileShare]::Delete)) }",
        "  try {",
        "    Set-CccAcquireStage 'hyper-v-base-image-partial-hash-failed'",
        "    $ValidatedPartialHash = if ($CompareTargetGuard) { Get-CccGuardedSha256 $CompareTargetGuard } else { (Get-FileHash -LiteralPath $PartialPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant() }",
        "    if ($ValidatedPartialHash -ne $PartialHashBefore) { throw 'hyper-v-base-image-partial-mutated' }",
        "    if ($CompareTargetExpectedHash -and $ValidatedPartialHash -ne $CompareTargetExpectedHash) { throw 'hyper-v-base-image-partial-mutated' }",
        "  } finally { if ($PartialGuard) { $PartialGuard.Dispose(); $PartialGuard = $null } }",
        "  Assert-NoReparsePath $PartialPath",
        "  Assert-NoReparsePath $ImagePath",
        "  Set-CccAcquireStage 'hyper-v-base-image-final-move-failed'",
        "  if ($CompareTargetGuard) {",
        "    $FinalGuard = [IO.File]::Open($ImagePath, [IO.FileMode]::CreateNew, [IO.FileAccess]::ReadWrite, [IO.FileShare]::Read)",
        "    $CompareTargetGuard.Position = 0",
        "    $CompareTargetGuard.CopyTo($FinalGuard, 8388608)",
        "    $FinalGuard.Flush($true)",
        "    $PublishedWriteHash = Get-CccGuardedSha256 $FinalGuard",
        "    if ($PublishedWriteHash -ne $ValidatedPartialHash -or ($CompareTargetExpectedHash -and $PublishedWriteHash -ne $CompareTargetExpectedHash)) { throw 'hyper-v-base-image-final-hash-mismatch' }",
        ...
        "    $PublishedReadHash = Get-CccGuardedSha256 $FinalGuard",
        "    if ($PublishedReadHash -ne $PublishedWriteHash) { throw 'hyper-v-base-image-final-hash-mismatch' }",
        "  } else {",
        ...
        "  $FinalHashBefore = if ($FinalGuard) { Get-CccGuardedSha256 $FinalGuard } else { (Get-FileHash -LiteralPath $ImagePath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant() }",
        "  if ($FinalHashBefore -ne $ValidatedPartialHash) { throw 'hyper-v-base-image-final-hash-mismatch' }",
        "  if ($CompareTargetExpectedHash -and $FinalHashBefore -ne $CompareTargetExpectedHash) { throw 'hyper-v-base-image-final-hash-mismatch' }",
        ...
        "  if ($CompareSourceGuard) { $CompareSourceGuard.Dispose(); $CompareSourceGuard = $null }",
        "  if ($CompareTargetGuard) { $CompareTargetGuard.Dispose(); $CompareTargetGuard = $null }",
