...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "    if ([string]$SourceInfo.format -ne 'qcow2' -or [long]$SourceInfo.'virtual-size' -le 0 -or [long]$SourceInfo.'virtual-size' -gt $UbuntuVirtualSizeBytes) { throw 'hyper-v-base-image-source-format-invalid' }",
        ...
        "    & $QemuImg convert -f qcow2 -O vhdx -o subformat=dynamic $SourceImagePath $QemuOutputPath 2>$null",
        "    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $QemuOutputPath -PathType Leaf)) { throw 'hyper-v-base-image-convert-failed' }",
        "    $ConvertedInfoText = & $QemuImg info --output=json $QemuOutputPath 2>$null",
        ...
        "    if ([string]$ConvertedInfo.format -ne 'vhdx' -or [long]$ConvertedInfo.'virtual-size' -le 0 -or [long]$ConvertedInfo.'virtual-size' -gt $UbuntuVirtualSizeBytes) { throw 'hyper-v-base-image-convert-failed' }",
        "    Set-CccAcquireStage 'hyper-v-base-image-source-hash-failed'",
        ...
        "    $NormalizedHashBeforeResize = (Get-FileHash -LiteralPath $NormalizedQemuPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()",
        "    if ($NormalizedHashBeforeResize -ne $QemuOutputHash) { throw 'hyper-v-base-image-normalize-failed' }",
        "    Resize-VHD -Path $NormalizedQemuPath -SizeBytes $UbuntuVirtualSizeBytes -ErrorAction Stop",
