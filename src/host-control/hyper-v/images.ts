...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "    $RequiredExtractionBytes = [long]$TotalExtractedBytes + [long]$ExpectedVhdBytes + [long]2GB",
        ...
        "    Protect-CccImageDirectory $ProfileRoot",
        "    $SourceGuard = [IO.File]::Open($SourcePath, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::ReadWrite)",
        "    try {",
        "      $SourceHashBefore = (Get-FileHash -LiteralPath $SourcePath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()",
        ...
        "    } finally { $SourceGuard.Dispose(); $SourceGuard = $null }",
        "  }",
        "  Protect-CccImageDirectory $ProfileRoot",
        "  if ($Profile -eq 'windows-server') { Set-CccAcquireStage 'hyper-v-base-image-inspection-failed' }",
        "  $PartialGuard = [IO.File]::Open($PartialPath, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::ReadWrite)",
        "  try {",
