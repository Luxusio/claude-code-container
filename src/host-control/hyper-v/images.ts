...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "  if ($Stage -notmatch '^hyper-v-base-image-(download|hash|archive-check|extract|normalize|native-finalize|source-open|source-hash|source-inspection|copy|destination-create|convert|partial-open|partial-hash|partial-inspection|final-move|final-inspection|final-observation)-failed$') { throw 'hyper-v-diagnostic-stage-invalid' }",
        ...
        "    $NormalizedHashBefore = (Get-FileHash -LiteralPath $NormalizedQemuPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()",
        "    Remove-Item -LiteralPath $QemuOutputPath -Force -ErrorAction Stop",
        "    Set-CccAcquireStage 'hyper-v-base-image-native-finalize-failed'",
        "    Assert-NoReparsePath $NormalizedQemuPath",
        "    Assert-NoReparsePath $PartialPath",
        "    Convert-VHD -Path $NormalizedQemuPath -DestinationPath $PartialPath -VHDType Dynamic -ErrorAction Stop",
        "    $NormalizedHashAfter = (Get-FileHash -LiteralPath $NormalizedQemuPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()",
        "    if ($NormalizedHashAfter -ne $NormalizedHashBefore) { throw 'hyper-v-base-image-native-finalize-failed' }",
        "    $NativeVhd = Get-VHD -Path $PartialPath -ErrorAction Stop",
        "    if ([long]$NativeVhd.Size -ne $UbuntuVirtualSizeBytes) { throw 'hyper-v-base-image-native-finalize-failed' }",
        "    Remove-Item -LiteralPath $NormalizedQemuPath -Force -ErrorAction Stop",
        "  }",
        "  Protect-CccImageDirectory $ProfileRoot",
