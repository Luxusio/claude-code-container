...
export function hyperVAcquireBaseImageCommand(options: HyperVAcquireBaseImageOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "  if ($Stage -notmatch '^hyper-v-base-image-(download|hash|archive-check|extract|normalize|source-open|source-hash|source-inspection|copy|destination-create|convert|efi-fallback|partial-open|partial-hash|partial-inspection|final-move|final-inspection|final-observation)-failed$') { throw 'hyper-v-diagnostic-stage-invalid' }",
        ...
        "$PartialGuard = $null",
        "$MountedUbuntuVhd = $false",
        "$EfiAccessPathAdded = $false",
        "$UbuntuMountAttempted = $false",
        "$EfiDetachVerified = $false",
        "$FinalEfiCleanupFailed = $false",
        "$EfiPartitions = @()",
        "$EfiAccessPath = ''",
        "try {",
        ...
        "    $EfiMountPath = Join-Path $WorkPath 'efi-mount'",
        "    New-Item -ItemType Directory -Path $EfiMountPath -ErrorAction Stop | Out-Null",
        "    $EfiAccessPath = $EfiMountPath.TrimEnd('\\') + '\\'",
        "    $EfiPreparationFailure = $null",
        "    try {",
        "      $UbuntuMountAttempted = $true",
        "      Mount-VHD -Path $QemuOutputPath -NoDriveLetter -ErrorAction Stop | Out-Null",
        "      $MountedUbuntuVhd = $true",
        "      $MountedDisk = Get-DiskImage -ImagePath $QemuOutputPath -ErrorAction Stop | Get-Disk -ErrorAction Stop",
        "      $EfiPartitions = @($MountedDisk | Get-Partition -ErrorAction Stop | Where-Object { [string]$_.GptType -eq '{c12a7328-f81f-11d2-ba4b-00a0c93ec93b}' })",
        "      if ($EfiPartitions.Count -ne 1) { throw 'hyper-v-base-image-efi-partition-invalid' }",
        "      Add-PartitionAccessPath -InputObject $EfiPartitions[0] -AccessPath $EfiAccessPath -ErrorAction Stop",
        "      $EfiAccessPathAdded = $true",
        "      $UbuntuEfiPath = Join-Path $EfiAccessPath 'EFI\\ubuntu'",
        "      $UbuntuShimPath = Join-Path $UbuntuEfiPath 'shimx64.efi'",
        "      if (-not (Test-Path -LiteralPath $UbuntuShimPath -PathType Leaf)) { throw 'hyper-v-base-image-efi-loader-missing' }",
        "      $FallbackEfiPath = Join-Path $EfiAccessPath 'EFI\\boot'",
        "      if (Test-Path -LiteralPath $FallbackEfiPath) { Remove-Item -LiteralPath $FallbackEfiPath -Recurse -Force -ErrorAction Stop }",
        "      Copy-Item -LiteralPath $UbuntuEfiPath -Destination $FallbackEfiPath -Recurse -Force -ErrorAction Stop",
        "      $FallbackShimPath = Join-Path $FallbackEfiPath 'shimx64.efi'",
        "      $FallbackBootPath = Join-Path $FallbackEfiPath 'bootx64.efi'",
        "      if (-not (Test-Path -LiteralPath $FallbackShimPath -PathType Leaf)) { throw 'hyper-v-base-image-efi-loader-copy-failed' }",
        "      Move-Item -LiteralPath $FallbackShimPath -Destination $FallbackBootPath -Force -ErrorAction Stop",
        "      if (-not (Test-Path -LiteralPath $FallbackBootPath -PathType Leaf)) { throw 'hyper-v-base-image-efi-fallback-missing' }",
        "    } catch { $EfiPreparationFailure = $_ } finally {",
        "      if ($EfiAccessPathAdded) { try { Remove-PartitionAccessPath -InputObject $EfiPartitions[0] -AccessPath $EfiAccessPath -ErrorAction Stop; $EfiAccessPathAdded = $false } catch { } }",
        "      if ($MountedUbuntuVhd) { try { Dismount-VHD -Path $QemuOutputPath -ErrorAction Stop; $MountedUbuntuVhd = $false; $EfiAccessPathAdded = $false } catch { } }",
        "      try { $EfiDetachVerified = (-not [bool](Get-DiskImage -ImagePath $QemuOutputPath -ErrorAction Stop).Attached) -and (-not (Test-Path -LiteralPath (Join-Path $EfiAccessPath 'EFI'))) } catch { $EfiDetachVerified = $false }",
        "    }",
        "    if ($MountedUbuntuVhd -or $EfiAccessPathAdded -or -not $EfiDetachVerified) { throw 'hyper-v-base-image-efi-cleanup-failed' }",
        "    $UbuntuMountAttempted = $false",
        "    if ($EfiPreparationFailure) { throw $EfiPreparationFailure }",
        ...
        "  if ($EfiAccessPathAdded) { try { Remove-PartitionAccessPath -InputObject $EfiPartitions[0] -AccessPath $EfiAccessPath -ErrorAction Stop; $EfiAccessPathAdded = $false } catch { } }",
        "  if ($MountedUbuntuVhd) { try { Dismount-VHD -Path $QemuOutputPath -ErrorAction Stop; $MountedUbuntuVhd = $false; $EfiAccessPathAdded = $false } catch { } }",
        "  if ($UbuntuMountAttempted) { try { $EfiDetachVerified = (-not [bool](Get-DiskImage -ImagePath $QemuOutputPath -ErrorAction Stop).Attached) -and (-not (Test-Path -LiteralPath (Join-Path $EfiAccessPath 'EFI'))) } catch { $EfiDetachVerified = $false } }",
        "  if ($MountedUbuntuVhd -or $EfiAccessPathAdded -or ($UbuntuMountAttempted -and -not $EfiDetachVerified)) { $FinalEfiCleanupFailed = $true }",
