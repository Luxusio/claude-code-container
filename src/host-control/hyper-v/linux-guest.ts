...
export function hyperVLinuxSeedCommand(options: HyperVLinuxSeedOptions): HyperVProviderCommand {
    ...
    return command(options.executable, jsonScript([
        ...
        "$UserData = @('#cloud-config', ('hostname: ' + $ExpectedName), 'manage_etc_hosts: true', ('user: ' + $GuestUsername), 'ssh_pwauth: false', 'disable_root: true', 'ssh_deletekeys: true', 'ssh_keys:', '  ed25519_private: |', $HostPrivateKeyYaml, ('  ed25519_public: ' + $HostPublicKeyText), 'package_update: false', 'users:', '  - default', ('  - name: ' + $GuestUsername), '    groups: [adm, sudo]', '    sudo: ALL=(ALL) NOPASSWD:ALL', '    shell: /bin/bash', '    lock_passwd: true', '    ssh_authorized_keys:', ('      - ' + $PublicKeyText), 'runcmd:', '  - [systemctl, enable, ssh]', '  - [systemctl, restart, ssh]', '') -join [Environment]::NewLine",
