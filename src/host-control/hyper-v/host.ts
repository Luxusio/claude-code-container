...
export function hyperVRebootCommand(options: HyperVRebootOptions): HyperVProviderCommand {
    return command(options.executable, jsonScript([
        ...
        "  try { Start-VM -VM $Vm -ErrorAction Stop | Out-Null } catch { throw 'hyper-v-reboot-start-failed' }",
        "} elseif ($Vm.State -eq 'Running') {",
        "  try { Restart-VM -VM $Vm -Force:$Force -Confirm:$false -ErrorAction Stop | Out-Null } catch { throw 'hyper-v-reboot-command-failed' }",
...
function hyperVEnsureNetworkScript(options: HyperVNetworkOptions): string {
    ...
    return jsonScript([
        ...
        "$RepairPersistedSwitchMarker = $false",
        "$RepairedPersistedSwitchMarker = $false",
        "$OriginalSwitchMarker = ''",
        ...
        "    $ObservedNotes = $Switches[0].Notes",
        "    $ObservedMarker = if ($null -eq $ObservedNotes) { '' } else { [Convert]::ToString($ObservedNotes, [Globalization.CultureInfo]::InvariantCulture) }",
        "    Set-CccHyperVNetworkStage 'hyper-v-network-marker-classification-failed'",
        "    $ObservedStable = $ObservedMarker -ceq $StableMarker",
        "    $ObservedToken = $false",
        "    $ObservedTokenValue = ''",
        "    if (-not $ObservedStable -and -not [string]::IsNullOrEmpty($ObservedMarker) -and $ObservedMarker.Length -eq ($MarkerPrefix.Length + 24)) {",
        ...
        "    $ObservedMarkerRecognized = $ObservedStable -or $ObservedToken",
        "    $CanRepairPersistedMarker = $AllowPersistedCccIdentityRepair -and $ExpectedSwitchId -and $ExpectedNatInstanceId",
        "    if (-not $ObservedMarkerRecognized -and -not $CanRepairPersistedMarker) { throw 'hyper-v-network-switch-ownership-conflict' }",
        "    if ($AllowPersistedCccIdentityRepair) {",
        "      Set-CccHyperVNetworkStage 'hyper-v-network-identity-evidence-inspection-failed'",
        "      if (-not $ExpectedSwitchId -or -not $ExpectedNatInstanceId) { throw 'hyper-v-network-switch-ownership-conflict' }",
        "      if (-not $ObservedMarkerRecognized) {",
        "        $RepairPersistedSwitchMarker = $true",
        "        $OriginalSwitchMarker = $ObservedMarker",
        "      } else {",
        ...
        "        if (-not ($TokenToStable -or $StableToToken)) { throw 'hyper-v-network-switch-ownership-conflict' }",
        ...
        "$RequiresMutation = ($Switches.Count -eq 0) -or (-not $GatewayExists) -or ($Nats.Count -eq 0) -or $RepairPersistedSwitchMarker",
        ...
        ...(allowPersistedCccIdentityRepair ? [
            "  if ($RepairPersistedSwitchMarker) {",
            "    Set-CccHyperVNetworkStage 'hyper-v-network-persisted-marker-repair-failed'",
            "    Set-VMSwitch -VMSwitch $Switch -Notes $Marker -ErrorAction Stop",
            "    $RepairedPersistedSwitchMarker = $true",
            "    $RepairedSwitches = @(Get-VMSwitch -Id ([Guid]$ExpectedSwitchId) -ErrorAction Stop)",
            "    if ($RepairedSwitches.Count -ne 1 -or $RepairedSwitches[0].Id.ToString().ToLowerInvariant() -cne $ExpectedSwitchId -or [string]$RepairedSwitches[0].Notes -cne $Marker) { throw 'hyper-v-network-persisted-marker-repair-failed' }",
            "    $Switch = $RepairedSwitches[0]",
            "  }",
        ] : []),
        ...
        "} catch {",
        ...(allowPersistedCccIdentityRepair ? [
            "  if ($RepairedPersistedSwitchMarker) {",
            "    Set-CccHyperVNetworkStage 'hyper-v-network-persisted-marker-rollback-failed'",
...
export function hyperVCleanupNetworkCommand(options: HyperVNetworkCleanupOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "  if ($RemoveSwitch) { try { $Attached = @(Get-VMNetworkAdapter -All -ErrorAction Stop | Where-Object { $_.SwitchName -eq $SwitchName }) } catch { throw 'hyper-v-network-switch-attachment-inspection-failed' }; if ($Attached.Count -gt 0) { $DeferredReason = 'hyper-v-network-switch-in-use' } }",
        "}",
        "if ($DeferredReason) { $Result = [ordered]@{ ok = $true; removedSwitch = $false; removedNat = $false; removedGateway = $false; alreadyMissing = $false; deferred = $true; reason = $DeferredReason }; $Result | ConvertTo-Json -Compress -Depth 5; return }",
        "$IsAdministrator = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)",
        "$RequiresMutation = ($Switches.Count -eq 1 -and ($RemoveSwitch -or $RemoveGateway)) -or ($Nats.Count -eq 1 -and $RemoveNat)",
        "if ($RequiresMutation -and -not $IsAdministrator) { throw 'hyper-v-network-elevation-required' }",
        "if ($Nats.Count -eq 1 -and $RemoveNat) { if ([string]$Nats[0].InternalIPInterfaceAddressPrefix -ne $Prefix) { throw 'hyper-v-network-nat-prefix-conflict' }; if ([string]$Nats[0].InstanceID -cne $ExpectedNatInstanceId) { throw 'hyper-v-network-nat-identity-conflict' } }",
        "if ($Switches.Count -eq 1 -and $RemoveSwitch) {",
        "  try { $Attached = @(Get-VMNetworkAdapter -All -ErrorAction Stop | Where-Object { $_.SwitchName -eq $SwitchName }) } catch { throw 'hyper-v-network-switch-attachment-inspection-failed' }",
        "  if ($Attached.Count -gt 0) { $Result = [ordered]@{ ok = $true; removedSwitch = $false; removedNat = $false; removedGateway = $false; alreadyMissing = $false; deferred = $true; reason = 'hyper-v-network-switch-in-use' }; $Result | ConvertTo-Json -Compress -Depth 5; return }",
        "  Remove-VMSwitch -VMSwitch $Switches[0] -Force -Confirm:$false -ErrorAction Stop",
        "  $RemovedSwitch = $true",
        "}",
        "if ($Nats.Count -eq 1 -and $RemoveNat) { Remove-NetNat -InputObject $Nats[0] -Confirm:$false -ErrorAction Stop; $RemovedNat = $true }",
