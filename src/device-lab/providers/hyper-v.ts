...
function elevatedNetworkCommand(executable: string, networkScript: string): HyperVProviderCommand {
    ...
    const innerScript = [
        ...
        "} finally {",
        "  try { $Writer.Write(($Envelope | ConvertTo-Json -Compress -Depth 5)); $Writer.Dispose(); $Pipe.Dispose() } catch {}",
        "}",
    ...
    const outerScript = [
        ...
        "  if (-not $Wait.AsyncWaitHandle.WaitOne(120000)) { try { if ($Child -and -not $Child.HasExited) { $Child.Kill() } } catch {}; throw 'hyper-v-network-pipe-handshake-timeout' }",
...
export function hyperVSetupCommand(executable: string): HyperVProviderCommand {
    ...
    const innerScript = [
        ...
        "  $WatchdogSource = \"Start-Sleep -Seconds 780; `$Target = Get-Process -Id $PID -ErrorAction SilentlyContinue; if (`$Target -and `$Target.StartTime.ToUniversalTime().Ticks -eq $SelfStartTicks) { Stop-Process -Id $PID -Force -ErrorAction SilentlyContinue }\"",
    ...
    const outerScript = [
        ...
        "$InnerSource = $InnerSource.Replace('__CCC_HYPER_V_SETUP_PIPE_NAME__', $PipeName).Replace('__CCC_HYPER_V_SETUP_USER_SID__', $SetupUserSid)",
        "$InnerEncoded = [Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($InnerSource))",
        "$InnerSource = $null",
        "try {",
        "  $Wait = $Pipe.BeginWaitForConnection($null, $null)",
