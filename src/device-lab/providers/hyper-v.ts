...
function isoWriterLines(): string[] {
    return [
        ...
        "    [Console]::Out.WriteLine('hyper-v-provisioning-media-configure-failed')",
        "    try {",
        ...
        "      if ($CauseMessage -match '\\b(hyper-v-provisioning-media-(?:block-invalid|stream-invalid|output-open-failed|copy-incomplete|com-unavailable|configure-failed|source-entry-invalid|source-directory-failed|source-file-invalid|source-file-failed|add-tree-failed|result-image-failed)\\b') { $CccIsoFailure = [string]$Matches[0]; break }",
