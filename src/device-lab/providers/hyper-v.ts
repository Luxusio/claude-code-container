...
function isoWriterLines(): string[] {
    return [
        ...
        "  private static extern int CreateStreamOnHGlobal(IntPtr memory, bool deleteOnRelease, out IStream stream);",
        "  public static IStream CreateSource(byte[] bytes) {",
        "    if (bytes == null || bytes.Length == 0 || bytes.Length > 1048576) throw new ArgumentOutOfRangeException(\"bytes\", \"hyper-v-provisioning-media-source-entry-invalid\");",
        "    IStream stream = null;",
        "    IntPtr writtenPointer = IntPtr.Zero;",
        "    IntPtr positionPointer = IntPtr.Zero;",
        "    try {",
        "      writtenPointer = Marshal.AllocHGlobal(sizeof(int));",
        "      positionPointer = Marshal.AllocHGlobal(sizeof(long));",
        "      int result = CreateStreamOnHGlobal(IntPtr.Zero, true, out stream);",
        "      if (result < 0 || stream == null) throw new IOException(\"hyper-v-provisioning-media-source-stream-failed\");",
        "      Marshal.WriteInt32(writtenPointer, 0);",
        "      stream.Write(bytes, bytes.Length, writtenPointer);",
        "      if (Marshal.ReadInt32(writtenPointer) != bytes.Length) throw new EndOfStreamException(\"hyper-v-provisioning-media-source-stream-failed\");",
        "      stream.Seek(0, 0, positionPointer);",
        "      if (Marshal.ReadInt64(positionPointer) != 0) throw new IOException(\"hyper-v-provisioning-media-source-stream-failed\");",
        "      return stream;",
        "    } catch {",
        "      if (stream != null && Marshal.IsComObject(stream)) { try { Marshal.FinalReleaseComObject(stream); } catch {} }",
        "      throw;",
        "    } finally {",
        "      if (writtenPointer != IntPtr.Zero) Marshal.FreeHGlobal(writtenPointer);",
        "      if (positionPointer != IntPtr.Zero) Marshal.FreeHGlobal(positionPointer);",
        "    }",
        "  }",
        ...
        "}",
        "function Write-CccIso([Collections.IDictionary]$Files, [string]$IsoPath, [string]$VolumeName) {",
        ...
        "      try {",
        "        $SourceStream = [CccIsoStreamWriter]::CreateSource($EntryBytes)",
        "        if ($null -eq $SourceStream) { throw 'hyper-v-provisioning-media-source-stream-failed' }",
        "        [void]$SourceStreams.Add($SourceStream)",
        "        $ImageRoot.AddFile($EntryName, $SourceStream)",
        "      } catch { throw 'hyper-v-provisioning-media-source-stream-failed' }",
        "    }",
        ...
        "      if ($CauseMessage -match '\\b(hyper-v-provisioning-media-(?:block-invalid|stream-invalid|output-open-failed|copy-incomplete|com-unavailable|configure-failed|source-entry-invalid|source-stream-failed|result-image-failed)\\b') { throw [string]$Matches[0] }",
        "      $Cause = $Cause.InnerException",
        "    }",
        "    throw 'hyper-v-provisioning-media-create-failed'",
        "  } finally {",
        "    foreach ($ComObject in @($ImageStream, $ResultImage, $ImageRoot) + @($SourceStreams) + @($Image)) {",
        "      if ($null -ne $ComObject -and [Runtime.InteropServices.Marshal]::IsComObject($ComObject)) {",
        "        try { [Runtime.InteropServices.Marshal]::FinalReleaseComObject($ComObject) | Out-Null } catch {}",
        "      }",
        "    }",
        "    $ImageStream = $null; $ResultImage = $null; $ImageRoot = $null; $SourceStreams = $null; $Image = $null",
        "  }",
...
export function hyperVLinuxSeedCommand(options: HyperVLinuxSeedOptions): HyperVProviderCommand {
    ...
    return command(options.executable, jsonScript([
        ...
        "  }",
        ...isoWriterLines(),
        "  Write-CccIso $IsoFiles $SeedDisk 'cidata'",
        "  $IsoFiles = $null",
...
export function hyperVGuestProvisionCommand(options: HyperVGuestProvisionOptions): HyperVProviderCommand {
    ...
    const script = jsonScript([
        ...
        "$CccProvisionStage = 'input-validation'",
        ...
        ...isoWriterLines(),
        "  Write-CccIso $IsoFiles $ProvisioningMedia 'CCC_UNATTEND'",
        "  $IsoFiles = $null",
        "  $UnattendBytes = $null",
