...
function isoWriterLines(): string[] {
    return [
        ...
        "      output.Commit(0);",
        "    } finally {",
        "      if (output != null && Marshal.IsComObject(output)) Marshal.FinalReleaseComObject(output);",
        ...
        "    try { $Image.Root.AddTree($SourceRoot, $false) } catch { throw 'hyper-v-provisioning-media-source-tree-failed' }",
        ...
        "      if ($CauseMessage -match '\\b(hyper-v-provisioning-media-(?:block-invalid|stream-invalid|output-open-failed|copy-incomplete|com-unavailable|configure-failed|source-tree-failed|result-image-failed))\\b') { throw [string]$Matches[1] }",
