...
function isoWriterLines(): string[] {
    return [
        ...
        "          int blockBytes = 0;",
        "          while (blockBytes < blockSize) {",
        "            Marshal.WriteInt32(readPointer, 0);",
        "            input.Read(buffer, blockSize - blockBytes, readPointer);",
        "            int count = Marshal.ReadInt32(readPointer);",
        "            if (count <= 0 || count > blockSize - blockBytes) throw new EndOfStreamException(\"hyper-v-provisioning-media-read-incomplete\");",
        "            output.Write(buffer, 0, count);",
        "            blockBytes += count;",
        "          }",
        "        }",
        "        if (output.Length != checked((long)blockSize * totalBlocks)) throw new InvalidDataException(\"hyper-v-provisioning-media-length-mismatch\");",
        "        output.Flush(true);",
        "      }",
        ...
        "    [CccIsoStreamWriter]::Write($ResultImage.ImageStream, $IsoPath, [int]$ResultImage.BlockSize, [long]$ResultImage.TotalBlocks)",
        "  } catch {",
        ...
        "    if ($Cause -match '\\b(hyper-v-provisioning-media-(?:block-invalid|stream-invalid|read-incomplete|length-mismatch))\\b') { throw [string]$Matches[1] }",
