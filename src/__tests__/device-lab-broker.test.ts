...
describe("device-lab host broker daemon", () => {
    ...
    it("replaces a same-version broker whose only stale contract is Hyper-V lifecycle v17", async () => {
        ...
        const staleHyperVCapabilities = [
            ...currentCapabilities.filter((capability) => capability !== "hyper-v-vm-managed-auto-images-v18"),
            "hyper-v-vm-managed-auto-images-v17",
        ];
        ...
        try {
            ...
            expect(incompatibleStatus).toEqual(expect.objectContaining({
                missingCapabilities: ["hyper-v-vm-managed-auto-images-v18"],
            }));
        } finally {
            killSpy.mockRestore();
