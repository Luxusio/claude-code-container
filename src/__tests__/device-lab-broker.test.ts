...
describe("device-lab host broker daemon", () => {
    ...
    it("replaces a same-version broker missing the Hyper-V image acquisition contract", async () => {
        ...
        const staleHyperVCapabilities = currentCapabilities.filter((capability) =>
            capability !== "hyper-v-image-acquisition-stage-cache-v1");
        ...
        try {
            ...
            expect(incompatibleStatus).toEqual(expect.objectContaining({
                missingCapabilities: ["hyper-v-image-acquisition-stage-cache-v1"],
