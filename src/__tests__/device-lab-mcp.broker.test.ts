...
describe("device-lab MCP", () => {
        ...
        const server = createServer((req, res) => {
            ...
            if (req.url === "/status") {
                res.end(JSON.stringify({
                    ...
                    broker: {
                        implemented: [
                            ...REQUIRED_CCC_HOST_BROKER_CAPABILITIES.filter((capability) => capability !== "hyper-v-setup-network-v7"),
                            "hyper-v-setup-network-v6",
                        ],
                    },
                }));
                return;
        ...
        try {
            ...
            expect(result).toEqual(expect.objectContaining({
                ...
                launch: expect.objectContaining({
                    ...
                    error: "host-broker-incompatible",
                    compatibility: expect.objectContaining({
                        missingCapabilities: ["hyper-v-setup-network-v7"],
                    }),
                }),
            }));
        } finally {
    ...
    it.each([
        ...
        ["Hyper-V provider-bound automatic image finalization contract", "hyper-v-provider-image-finalization-v2"],
        ["Hyper-V redacted network failure diagnostics", "hyper-v-network-failure-diagnostics-v1"],
