...
describe("device-lab MCP", () => {
    ...
    it("rejects a stale v11 Hyper-V broker even when runtime metadata is missing", { timeout: TIMEOUT }, async () => {
        const server = createServer((req, res) => {
            res.setHeader("content-type", "application/json");
            if (req.url === "/health") {
                res.end(JSON.stringify({ ok: true, name: "ccc-device-broker", mode: "host-broker-daemon" }));
                    ...
                    broker: {
                        implemented: [
                            ...REQUIRED_CCC_HOST_BROKER_CAPABILITIES.filter((capability) =>
                                capability !== "hyper-v-vm-managed-auto-images-v12"),
                            "hyper-v-vm-managed-auto-images-v11",
                        ],
                    },
                }));
                return;
        ...
        try {
            ...
            expect(result).toEqual(expect.objectContaining({
                ...
                launch: expect.objectContaining({
                    ...
                    error: "host-broker-incompatible",
                    compatibility: expect.objectContaining({
                        missingCapabilities: ["hyper-v-vm-managed-auto-images-v12"],
                    }),
                }),
            }));
        } finally {
