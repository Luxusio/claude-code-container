...
describe("device-lab MCP", () => {
    ...
    it("rejects a same-version broker without Azure-compatible Hyper-V guest seeding", { timeout: TIMEOUT }, async () => {
        const server = createServer((req, res) => {
            res.setHeader("content-type", "application/json");
            if (req.url === "/health") {
                res.end(JSON.stringify({ ok: true, name: "ccc-device-broker", mode: "host-broker-daemon" }));
                ...
                res.end(JSON.stringify({
                    ...
                    broker: {
                        implemented: REQUIRED_CCC_HOST_BROKER_CAPABILITIES.filter((capability) =>
                            capability !== "hyper-v-azure-ovf-seed-v1"),
                    },
                }));
                return;
            }
        ...
        try {
            ...
            expect(result).toEqual(expect.objectContaining({
                ...
                launch: expect.objectContaining({
                    ...
                    error: "host-broker-incompatible",
                    compatibility: expect.objectContaining({
                        missingCapabilities: ["hyper-v-azure-ovf-seed-v1"],
                    }),
                }),
            }));
        } finally {
