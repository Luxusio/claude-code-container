...
65. Unattended Hyper-V Linux guests use Canonical's dated Ubuntu Server cloud
    ...
    The provider verifies the pinned VMDK checksum, resolves `qemu-img.exe`
    only from the current user's default Android SDK path, verifies its Google
    Authenticode signature and identity, converts Canonical's Hyper-V
    documented VMDK source to a
    ...
    VHDX bound to the catalog's fixed Generation 2 contract and Linux UEFI CA
    Secure Boot template. The QEMU-oriented QCOW source is not accepted.
    Brokers advertise `hyper-v-provider-image-finalization-v18`, preventing a
...
67. Automatic Hyper-V image acquisition holds source and partial VHDX file
    ...
    in-place changes,
    the source VMDK is SHA-256 checked before and after conversion and the
    ...
    `hyper-v-provider-image-finalization-v18`
...
70. Automatic Hyper-V image profiles no longer mount the prepared VHDX merely
    ...
    `hyper-v-provider-image-finalization-v18` for this contract.
