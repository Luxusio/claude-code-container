...
- The same file-pointer fallback applies to macOS Finder file URL clipboard payloads and Linux desktop file URI clipboard payloads (`text/uri-list` and GNOME copied-files style data): direct clipboard image bytes remain first priority, readable local image file bytes are second priority, and shared copied file paths under `/run/ccc/clipboard-files/...` are last resort.
...
- Copied local image files from macOS Finder or Linux desktops also paste as readable file bytes before falling back to a container-accessible copied file path under the CCC clipboard shared-files mount.
...
## Verification Cues
- Focused tests should prove alternate image MIME ordering, Darwin helper JSON parsing, Windows image-file snapshot parsing, Linux file URI parsing, readable file-byte priority, copied container-path fallback parsing, clipboard shared-files mount arguments, and preservation of normal non-image text behavior. Build verification should compile the TypeScript clipboard server and package the updated Darwin helper source.
