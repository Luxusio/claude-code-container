...
14. Host-backed virtualized or physical devices should keep authoritative
    runtime state on the host side. Container-local MCP state may be used only
    as a transient cache or compatibility bridge, because container removal
    must not orphan Windows Sandbox, Android Emulator, iOS Simulator, macOS VM,
    Appium, or recording processes.
15. Appium is an automation bridge, not a virtualization provider. CCC may
    install Appium components for zero-configuration automation, but iOS and
    host-virtualized Android automation should prefer host-broker-managed
    Appium sessions bound to host-visible simulators, emulators, or physical
    devices. Container-local Appium is acceptable only for explicit fallback
    paths where the device transport is reachable from the container and no
    host process ownership would be lost.
...
- Treating container-local Appium metadata as authoritative host device state.
